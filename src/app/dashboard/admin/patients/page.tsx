...
export default function PatientsManagement() {
  ...
  // Extract the patients from API response or use empty array as fallback
  const patients: Patient[] = data?.users || [];
  
  // Get pagination data from API or use defaults
  const totalPatients = data?.pagination?.totalUsers || 0;
          ...
          <div className="lg:col-span-3">
            <div className="relative">
              ...
              <input
                ...
                className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full pl-10 p-2.5 transition-all"
          ...
          <div>
            <div className="relative">
              ...
              <select
                ...
                className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full pl-10 p-2.5 appearance-none transition-all"
              >
      ...
      {isLoading || swrLoading ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 mb-6 flex justify-center items-center">
          <div className="text-center">
            <div className="relative">
              <div className="w-14 h-14 border-3 border-green-200 border-t-green-600 rounded-full animate-spin mx-auto mb-3"></div>
              <StarIconSolid className="w-5 h-5 text-green-600 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 animate-pulse" />
            </div>
            <p className="text-gray-600 font-medium text-sm mt-2">Loading patients...</p>
          </div>
        </div>
      ) : error ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 mb-6">
          <div className="text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Error Loading Patients</h3>
            <p className="text-gray-600 mb-4 max-w-md mx-auto">There was an error loading the patient data. Please try again later.</p>
            <button
              onClick={() => mutate()}
        ...
        <>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {patients.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-10 whitespace-nowrap text-center text-gray-500">
                    ...
                    patients.map((patient) => (
                      <tr key={patient.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-10 w-10">
                              {patient.profileImage ? (
                                <img className="h-10 w-10 rounded-full object-cover border border-gray-200" src={patient.profileImage} alt={patient.name} />
                              ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2.5 py-1 inline-flex text-xs leading-4 font-medium rounded-full ${getStatusColor(patient.status)}`}>
                            {patient.status}
                          </span>
                        </td>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between bg-white px-4 py-3 rounded-xl shadow-sm border border-gray-100 gap-4">
            <div className="flex items-center text-sm text-gray-700">
              Showing <span className="font-medium mx-1">{patients.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> to{' '}
              <span className="font-medium mx-1">{Math.min(currentPage * itemsPerPage, totalPatients)}</span> of{' '}
              <span className="font-medium mx-1">{totalPatients}</span> results
            </div>
