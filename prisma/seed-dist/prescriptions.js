...
async function seedPrescriptions() {
    ...
    for (const appointment of completedAppointments) {
        ...
        const prescription = {
            ...
            validUntil,
            status: Math.random() < 0.8 ? client_1.PrescriptionStatus.ACTIVE :
                Math.random() < 0.5 ? client_1.PrescriptionStatus.COMPLETED : client_1.PrescriptionStatus.EXPIRED
        };
        ...
        for (const prescription of allPrescriptions) {
            ...
            for (const medicine of selectedMedicines) {
                ...
                const item = {
                    ...
                    instructions: `Take ${dosages[Math.floor(Math.random() * dosages.length)]} ${frequencies[Math.floor(Math.random() * frequencies.length)].toLowerCase()} ${medicine.prescriptionRequired ? 'with food' : Math.random() < 0.5 ? 'with food' : 'on empty stomach'}. ${medicine.warnings ? 'Note: ' + medicine.warnings.split('.')[0] + '.' : ''}`,
